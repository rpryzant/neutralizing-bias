...
num_batches = len(src['data']) / batch_size
...
for epoch in range(start_epoch, config['training']['epochs']):
    ...
    for i in range(0, len(src['data']), batch_size):
        ...
        input_lines_src, _, srclens, srcmask, idx = data.get_minibatch(
            src['data'], src['tok2id'], i,
            batch_size, max_length, sort=True
        )
        input_lines_tgt, output_lines_tgt, _, _, idx = data.get_minibatch(
            tgt['data'], tgt['tok2id'], i,
            batch_size, max_length, idx=idx
        )
