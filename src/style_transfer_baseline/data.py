...
def read_nmt_data(src, config, tgt, attribute_vocab):
    ...
    tgt = {
        ...
        'tok2id': tgt_tok2id, 'id2tok': tgt_id2tok
    }
...
def minibatch(src, tgt, idx, batch_size, max_len, model_type):
    ...
    if model_type == 'delete':
        ...
        inputs = get_minibatch(
            dataset['content'], dataset['tok2id'], i, batch_size, max_len, sort=True)
            
        outputs = get_minibatch(
            dataset['data'], dataset['tok2id'], i, batch_size, max_len, idx=inputs[-1])
        ...
        inputs =  get_minibatch(
            src['content'], src['tok2id'], i, batch_size, max_len, sort=True)
            
        TODO!!! THE REPLACEMENT THING??
        attributes =  get_minibatch(
            src['attribute'], src['tok2id'], i, batch_size, max_len, idx=inputs[-1])

        outputs = get_minibatch(
            src['data'], src['tok2id'], i, batch_size, max_len, idx=inputs[-1])
